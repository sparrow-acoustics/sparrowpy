...
import numpy as np
import sparrowpy.form_factor.utils as utils
try:
    import numba
    prange = numba.prange
except ImportError:
...
def calc_form_factor(
    ...
    if utils.coincidence_check(
        ...
        out = stokes_integration(
            ...
            source_area=utils.polygon_area(source_pts), approx_order=4)
...
def stokes_integration(
    ...
    i_bpoints, i_conn = utils.sample_border(patch_i, npoints=approx_order+1)
    j_bpoints, j_conn = utils.sample_border(patch_j, npoints=approx_order+1)
    ...
    for dim in range(len(j_bpoints[0])):
        ...
        for i in range(len(i_bpoints)):
            ...
            for segj in j_conn:
                ...
                if xj[-1]-xj[0]!=0:
                    ...
                    quadfactors = utils.poly_estimation(x=xj, y=subsecj)
                    # analytical integration of the approx polynomia
                    inner_integral[i][dim] += utils.poly_integration(
                        c=quadfactors,x=xj)
        ...
        for segi in i_conn:
            ...
            if xi[-1]-xi[0]!=0:
                ...
                quadfactors = utils.poly_estimation(x=xi, y=subseci)
                outer_integral += utils.poly_integration(c=quadfactors,x=xi)
...
def nusselt_analog(
    ...
    boundary_points, connectivity = utils.sample_border(
    ...
    rotmat = utils.rotation_matrix(n_in=surf_normal)

    for ii in prange(len(sphPts)):
        # points on the hemisphere projected onto
        plnPts[ii,:] = utils.inner(matrix=rotmat,vector=sphPts[ii])[:-1]
        projPts[ii,:-1] = plnPts[ii,:]
        projPts[ii,-1] = 0.


    big_poly = utils.polygon_area(projPts[0::2])
    ...
    for jj in prange(connectivity.shape[0]):
        ...
        if np.linalg.norm(np.cross(
            ...
            if np.dot( plnPts[segmt[-1]], plnPts[segmt[0]] ) >= 0:
                curved_area += utils.area_under_curve(plnPts[segmt],order=2)
            ...
            # if points span over 90º, additional sampling is required
                ...
                mpoint = utils.inner(matrix=rotmat,vector=mpoint)[:-1]
                marc = utils.inner(matrix=rotmat,vector=marc)[:-1]
                a = a/np.linalg.norm(a)
                a = utils.inner(matrix=rotmat,vector=a)[:-1]

                b = b/np.linalg.norm(b)
                b = utils.inner(matrix=rotmat,vector=b)[:-1]
                ...
                left =  utils.area_under_curve(leftseg, order=2)
                right = utils.area_under_curve(rightseg, order=2)
...
def nusselt_integration(
    ...
    if random:
        p0_array = utils.sample_random(patch_i,nsamples)
    else:
        p0_array = utils.sample_regular(patch_i,nsamples)
    ...
    out *= utils.polygon_area(patch_i)/(
        np.pi * len(p0_array) * utils.polygon_area(patch_j))
...
def pt_solution(point: np.ndarray, patch_points: np.ndarray, mode='source'):
    ...
    if mode == 'receiver':
        source_area = utils.polygon_area(patch_points)
    elif mode == 'source':
        source_area = 4
    ...
    for i in range(npoints):
        ...
        v0 = utils.calculate_tangent_vector(
            patch_onsphere[i], patch_onsphere[(i-1)%npoints])
        v1 = utils.calculate_tangent_vector(
            patch_onsphere[i], patch_onsphere[(i+1)%npoints])
...
def calc_form_factor(source_pts: np.ndarray, source_normal: np.ndarray,
    ...
    """
    if utils.coincidence_check(receiver_pts, source_pts):
        out = nusselt_integration(
                    patch_i=source_pts, patch_i_normal=source_normal,
...
def nusselt_analog(surf_origin, surf_normal,
    ...
    """
    boundary_points, connectivity = utils.sample_border(patch_points,
                                                          npoints=3)
...
def pt_solution(point: np.ndarray, patch_points: np.ndarray, mode='source'):
    ...
    for i in range(npoints):
        ...
        v0 = utils.calculate_tangent_vector(patch_onsphere[i],
                                              patch_onsphere[(i-1)%npoints])
        v1 = utils.calculate_tangent_vector(patch_onsphere[i],
                                              patch_onsphere[(i+1)%npoints])
