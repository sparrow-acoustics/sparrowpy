...
def load_stokes_entries(
    i_bpoints: np.ndarray, j_bpoints: np.ndarray
) -> np.ndarray:
...
def stokes_integration(
    ...
    """
    i_bpoints, i_conn = _sample_boundary_regular(patch_i, npoints=5)
    j_bpoints, j_conn = _sample_boundary_regular(patch_j, npoints=5)
...
def nusselt_analog(
    surf_origin, surf_normal, patch_points, patch_normal
) -> float:
    ...
    for ii in prange(len(sphPts)):
        ...
        plnPts[ii, :] = geom._matrix_vector_product(
            matrix=rotmat, vector=sphPts[ii]
        )[:-1]
...
def _newton_cotes_4th(x: np.ndarray, y):
    ...
    h = x[1] - x[0]
    return (
        2 * h / 45 * (7 * y[0] + 32 * y[1] + 12 * y[2] + 32 * y[3] + 7 * y[4])
    )
...
def _surf_sample_regulargrid(el: np.ndarray, npoints=10):
    ...
    npointsx = int(
        round(np.linalg.norm(u) / np.linalg.norm(v) * np.sqrt(a * npoints))
    )
    npointsz = int(
        round(np.linalg.norm(v) / np.linalg.norm(u) * np.sqrt(a * npoints))
    )
...
if numba is not None:
    pt_solution = numba.njit(cache=True, parallel=True)(pt_solution)
    nusselt_integration = numba.njit(cache=True, parallel=False)(
        nusselt_integration
    )
    stokes_integration = numba.njit(cache=True, parallel=False)(
        stokes_integration
    )
    nusselt_analog = numba.njit(cache=True, parallel=False)(nusselt_analog)
    load_stokes_entries = numba.njit(cache=True, parallel=True)(
        load_stokes_entries
    )
    _poly_estimation_Lagrange = numba.njit(cache=True)(
    ...
    _area_under_curve = numba.njit(cache=True)(_area_under_curve)
