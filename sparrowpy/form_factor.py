...
def calc_form_factor(
    ...
    """
    returns the form factor depending on the characteristics of the
    input patches.
    ...
    if helpers.coincidence_check(receiving_pts, source_pts):
        ...
        out = stokes_integration(
            ...
            source_area=helpers.polygon_area(source_pts),
            approx_order=4)
...
def stokes_ffunction(p0:np.ndarray, p1: np.ndarray) -> float:
    """
    returns the form function value for the stokes form factor
    integration method.
...
def stokes_integration(
    ...
    for dim in range(len(j_bpoints[0])):
        ...
        # integrate previously computed integral over each boundary
        # segment of patch i
...
def nusselt_integration(
        patch_i: np.ndarray, patch_j: np.ndarray,
    ...
    (Nusselt analogue) over the surface of the source patch.
    ...
    out *= helpers.polygon_area(
        patch_i)/(np.pi * len(p0_array) * helpers.polygon_area(patch_j))
