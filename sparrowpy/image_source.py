...
def get_walls_hesse(room_length_x, room_length_y, room_length_z):
    """Get the walls of a room in Hesse normal form.

    Parameters
    ----------
    room_length_x : float
        The length of the room in the x-direction.
    room_length_y : float
        The length of the room in the y-direction.
    room_length_z : float
        The length of the room in the z-direction.

    Returns
    -------
    WallsHesse : np.ndarray
        An (n_walls, 4) array where each row represents a wall
...
def calculate_image_sources(walls_hesse, source_position, max_order):
    """Calculate image sources based on the walls and source position.

    Parameters
    ----------
    walls_hesse : np.ndarray
        An (n_walls, 4) array where each row represents a wall
        in Hesse normal form [n0x, n0y, n0z, d].
    ...
    -------
    ISList : list of dict
        List of image sources, where each source is a dictionary with keys:
        'Position': (np.ndarray) position of the image source
        'Walls': (list) list of wall indices that the source is reflected off
        'Order': (int) order of the image source
    """
    n_walls = walls_hesse.shape[0]
    n_IS = int(1 + n_walls / (n_walls - 2) * ((n_walls - 1) ** max_order - 1))

    ISList = [ob.ISObj() for _ in range(1,n_IS+1)]

    ISList[0].Position = source_position
    ISList[0].Order = 0
        ...
        for iISID in range(list_end + 1):
            if ISList[iISID].Order == order - 1:
                for iWalls in range(1,n_walls + 1):
                    ...
                    n0 = walls_hesse[iWalls-1, 0:3]
                    p = ISList[iISID].Position
                    d = walls_hesse[iWalls-1, 3]
                    t0 = d - np.dot(n0, p)
                    ...
                    if wallCriterium and (t0 > 0):
                        ...
                        ISList[list_end + 1].Walls = ISList[iISID].Walls+[
                            iWalls]
...
def get_corners_of_walls_hesse(walls_hesse):
    """
    Compute the corners of walls based on their Hesse normal form
    representation.
...
def filter_image_sources(ISList, walls_hesse, receiver_position):
    """
    ...
    Returns
    -------
    filtered_is : list of dict
        Filtered list of image sources.
    """
    corners = get_corners_of_walls_hesse(walls_hesse)
    ...
    for ind, mis in enumerate(ISList):
        ...
        a = receiver_position
        b = mis.Position - receiver_position

        for k in range(len(walls)-1, -1, -1):
            # PROBLEM HERE original was walls[k]. maybe walls[k-1] is correct?
            wall_hesse = walls_hesse[walls[k]-1,:]
            n0 = wall_hesse[0:3]
            ...
            # Ensure that a, bound_min, and bound_max are scalars;
            # otherwise, use np.any() or np.all()


            if any(
                    a[i] < bound_min[i] or a[i] > bound_max[i] \
                        for i in range(3)):
                filter_flags[ind] = False
                break
            ...
            # original was for ind2 in range(n_walls)
            for ind2 in range(1,n_walls+1):
                if ind2 == except_wall:
                    continue

                wall = walls_hesse[ind2-1,:] #original was WallsHesse[ind2,:]
...
def calculate_impulse_response(ISList, WallsR, ReceiverPos):
    """Calculate the impulse response based on the image sources."""
    c0 = 346.18
