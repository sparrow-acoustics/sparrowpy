...
class DirectionalRadiosityFast:
    ...
    def bake_geometry(self, integration_args=None):
    ...
    def calculate_direct_sound(self, receivers):
            ...
            for i in range(self.n_bins):
                direct_sound[:, i] *= np.real(
                    self._source.get_directivity(
                        np.squeeze(receivers.cartesian),
                        self._frequencies[i],
                    ),
                )
        ...
        n_sample_delay = np.array(
            r / self.speed_of_sound / self._etc_time_resolution,
            dtype=int,
        )
