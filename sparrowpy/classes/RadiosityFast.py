...
class DirectionalRadiosityFast:
    ...
    def __init__(
        ...
        patch_to_wall_ids = np.atleast_1d(
            np.array(patch_to_wall_ids, dtype=int)
        )
        ...
        if patch_2_brdf_outgoing_index is not None:
            patch_2_brdf_outgoing_index = np.array(
                patch_2_brdf_outgoing_index, dtype=np.int64
            )
        if brdf is not None:
    ...
    def check(self):
        ...
        if any(
            i not in np.arange(n_walls) for i in set(self._patch_to_wall_ids)
        ):
            raise ValueError(
                "patch_to_wall_ids does contain other ids than range(n_walls)"
            )
        if any(
            i not in set(self._patch_to_wall_ids) for i in np.arange(n_walls)
        ):
            raise ValueError(
                "patch_to_wall_ids does contain other ids than range(n_walls)"
            )
        ...
        if self._frequencies is not None:
            if len(self._frequencies.shape) != 1:
                raise ValueError("Frequencies need to be of shape (n_bins,)")
        ...
        if self._brdf_index is not None:
            if len(self._brdf_index) != n_walls:
                raise ValueError("brdf_index need to be of shape (n_walls,)")
        if self._brdf_incoming_directions is not None:
            if any(
                not isinstance(i, pf.Coordinates)
        ...
        if self._energy_exchange_etc is not None:
            n_samples = int(self._etc_duration / self._etc_time_resolution)
    ...
    def from_polygon(cls, polygon_list, patch_size):
        ...
        # create patches
        (patches_points, _, n_patches, patch_to_wall_ids) = (
            geometry._process_patches(
    ...
    def from_file(
        ...
        geom_data = blender.read_geometry_file(
            filepath,
    ...
    def bake_geometry(self):
        ...
        if self._brdf_incoming_directions is not None:
            ...
            self._patch_2_brdf_outgoing_index = receivers_array.shape[
                1
            ...
            for j in range(self.n_patches):
                ...
                self._patch_2_brdf_outgoing_index[vis, j] = (
                    get_scattering_data_receiver_index(
                        pos_i=self.patches_center[vis],
                        pos_j=self.patches_center[j],
            ...
            self._patch_2_brdf_outgoing_index = np.zeros(
                (self.n_patches, self.n_patches), dtype=np.int64
            )

        n_bins = 1 if self._frequencies is None else self.n_bins

        self._form_factors_tilde = _form_factors_with_directivity_dim(
    ...
    def init_source_energy(self, source):
        ...
        if self._air_attenuation is None:
            ...
            self._frequencies = frequencies
        n_bins = self.n_bins
        vi = np.array([s.cartesian for s in self._brdf_incoming_directions])
        vo = np.array([s.cartesian for s in self._brdf_outgoing_directions])
        ...
        if isinstance(source, sound_object.SoundSource):
            ...
            if source.directivity is not None:
                ...
                for i_frequency in range(n_bins):
                    directivity_local = np.real(
                        source.get_directivity(
    ...
    def calculate_energy_exchange(
        ...
        if self._energy_exchange_etc is None or recalculate:
            ...
            if max_reflection_order < 1:
                self._energy_exchange_etc = _energy_exchange_init_energy(
                    n_samples,
                    energy_0_dir,
                    distance_0,
                    speed_of_sound,
    ...
    def collect_energy_receiver_mono(self, receivers, direct_sound=False):
        ...
        if not isinstance(direct_sound, bool):
            raise ValueError("direct_sound must be of type boolean")
    ...
    def calculate_direct_sound(self, receivers):
        ...
        direct_sound = np.ones((receivers.cshape[0], self.n_bins), dtype=float)
        direct_sound *= (1 / (4 * np.pi * r**2))[:, np.newaxis]

        # add air attenuation
        if self._air_attenuation is not None:
            for i in range(self.n_bins):
                direct_sound[:, i] *= np.exp(-self._air_attenuation[i] * r)

        # add source directivity
        if isinstance(self._source, sound_object.SoundSource):
            for i in range(self.n_bins):
                direct_sound[:, i] *= np.real(
                    self._source.get_directivity(
    ...
    def _collect_energy_patches(self, receiver_pos, propagation_fx=False):
        ...
        for i in range(n_receivers):
            ...
            patch_receiver_energy = (
                form_factor._patch2receiver_energy_universal(
...
def _add_directional(
    ...
    for i in prange(n_patches):
        ...
        energy_0_directivity[i, :, :] = energy_0[i] * np.real(
            scattering_factor
        )
            ...
            for jj in range(2):
                ...
                if n_delay_samples > 0:
                    E_matrix[current_index, j, :, :, n_delay_samples:] += (
                        form_factors_tilde[i, j]
                        * E_matrix[
                            current_index - 1, i, dir_id, :, :-n_delay_samples
                        ]
                    )
                else:
                    E_matrix[current_index, j, :, :, :] += (
                        form_factors_tilde[i, j]
                        * E_matrix[current_index - 1, i, dir_id, :, :]
                    )
    ...
    for i in prange(n_patches):
        ...
        for j in range(n_bins):
            E_mat_out[i, j] = np.roll(
                E_matrix_total[i, j]
                * np.exp(-air_attenuation[j] * patch_receiver_distance[i]),
                n_delay_samples,
            )
...
def _form_factors_with_directivity(
    ...
    for ii in prange(n_patches**3):
        ...
        visible_hi = (
            visibility_matrix[h, i] if h < i else visibility_matrix[i, h]
        )
        visible_ij = (
            visibility_matrix[i, j] if i < j else visibility_matrix[j, i]
        )
        if visible_hi and visible_ij:
            ...
            ff = (
                form_factors[i, j]
                if i < j
                else (form_factors[j, i] * patches_area[i] / patches_area[j])
            )
            ...
            if air_attenuation is not None:
                form_factors_tilde[h, i, j, :] = form_factors_tilde[
                    h, i, j, :
                ] * np.exp(-air_attenuation * distance)
            ...
            if scattering is not None:
                ...
                form_factors_tilde[h, i, j, :] = (
                    form_factors_tilde[h, i, j, :] * scattering_factor
                )
    ...
    for ii in prange(n_patches**2):
        ...
        if visible_ij:
            ...
            ff = (
                form_factors[i, j]
                if i < j
                else (form_factors[j, i] * patches_area[j] / patches_area[i])
            )
            ...
            if scattering is not None:
                ...
                form_factors_tilde[i, j, :, :] = (
                    form_factors_tilde[i, j, :, :] * scattering_factor
                )
...
def get_scattering_data_receiver_index(
    ...
    for i in range(n_patches):
        ...
        difference_receiver /= np.linalg.norm(difference_receiver)
        receiver_idx[i] = np.argmin(
            np.sum(
                (receivers[wall_id_i[i], :] - difference_receiver) ** 2,
                axis=-1,
            ),
            axis=-1,
        )
...
def get_scattering_data(
    ...
    difference_receiver /= np.linalg.norm(difference_receiver)
    source_idx = np.argmin(
        np.sum((sources[wall_id_i, :, :] - difference_source) ** 2, axis=-1)
    )
    receiver_idx = np.argmin(
        np.sum((receivers[wall_id_i, :] - difference_receiver) ** 2, axis=-1)
    )
    return scattering[scattering_index[wall_id_i], source_idx, receiver_idx, :]
...
if numba is not None:
    _add_directional = numba.njit(cache=True, parallel=True)(_add_directional)
    _energy_exchange_init_energy = numba.njit(cache=True)(
    ...
    get_scattering_data = numba.njit(cache=True)(get_scattering_data)
    get_scattering_data_source = numba.njit(cache=True)(
