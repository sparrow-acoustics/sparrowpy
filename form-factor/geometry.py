...
class Polygon():
    ...
    def __init__(
        ...
        """
        self.pts = np.array(points, dtype=float) # it could be convenient to calculate a default normal (depends on order of the pts), 
        normal = np.array(normal, dtype=float)   # and introduce a default up vector (maybe relating to one of the sides of the patch) -- this way only the polygon vertices need to be explicitly stated
        self.form_factors = []                   # is it more convenient to 
        # check if points are in one plane
        assert self.pts.shape[1] >= 3, \
            'You need at least 3 points to build a Polygon' # what is going on here though
        if self.n_points > 3:
        ...
        assert all(abs(np.cross(normal, calc_normal)) <= 10**-15), \
