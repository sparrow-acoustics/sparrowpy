...
def perpendicular_patch_coincidentline(w,h,l):
    """
...
def perpendicular_patch_coincidentpoint(aa,bb,cc,dd):
    ...
    out  = (bb+dd)**2 * np.log(((c**2 + a**2*b**2 + 1)/(c**2 +1)) * ((b**2 + b**2*c**2)/(c**2 +c**2*b**2))**(b**2) * ((a**2 + a**2*c**2)/(c**2 +c**2*a**2))**(a**2))

    out += bb**2 * np.log(((f**2 + 1)/(f**2 + d**2*e**2 + 1)) * ((f**2 + f**2*e**2)/(e**2 + e**2*f**2))**(e**2) * ((f**2 + f**2*d**2)/(d**2 + d**2*f**2))**(d**2))

    out += dd**2 * np.log(((z**2 + 1)/(z**2 + x**2*y**2 + 1)) * ((z**2 + z**2*y**2)/(y**2 + y**2*z**2))**(y**2) * ((z**2 + z**2*x**2)/(x**2 + x**2*z**2))**(x**2))

    out += 4 * (bb+dd)**2 * (  a*np.arctan(1/a) + b*np.arctan(1/b) - c*np.arctan(1/c))

    out -= 4 * bb**2 * (  d*np.arctan(1/d) + e*np.arctan(1/e) - f*np.arctan(1/f)  )

    out -= 4 * dd**2 * (  x*np.arctan(1/x) + y*np.arctan(1/y) - z*np.arctan(1/z)  )
...
def parallel_patches(a,b,c):
    ...
    """
    returns the exact form factor solution of parallel patches facing eachother
    both patches have a length a and width b. They have a distance c from eachother.
    """
