...
plt.hlines(energy_analytical, np.min(n_integrations), np.max(n_integrations), colors='r', linestyles='--', label='Analytical')
plt.legend()

# %%
rel_error = np.abs((energy_radiosity.T - energy_analytical)/energy_analytical)
plt.figure()
