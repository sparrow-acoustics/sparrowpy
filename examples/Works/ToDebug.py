...
#sampling = sp.brdf.sph_gaussian_hemisphere(sh_order=sh_order) -> guaranteed energy conservation is exact 1 for diffuse and specular

sampling = pf.samplings.sph_gaussian(sh_order)
...
if True:
    #for a in range(sampling.cartesian.shape[0]):
    for a in range(1):
        sum = 0
        for i in range(sampling.cartesian.shape[0]):
            cos_theta = np.dot([0,0,1],sampling.cartesian[i])
            sum += brdf_data.freq[i,a] * cos_theta * sampling.weights[i]
...
radi_brdf_integration._integration_method = "montecarlo" # "leggauss" or "montecarlo"
radi_brdf_integration._integration_sampling = 200 #for montecarlo = N^2, for leggauss = N. leggauss max N = 30 -> already long computation. so montecarlo up to 1000 samplings on the surface
# set air absorption
