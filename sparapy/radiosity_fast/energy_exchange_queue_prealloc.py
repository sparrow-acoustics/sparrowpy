...
def _calculate_energy_exchange_queue(
    ...
    q_length = int( distance_i_j.shape[0] ** (np.ceil(np.log(threshold)/np.log(np.max(form_factors_tilde)))) )

    queue = np.zeros((q_length, 4))
    queue[0:indices.shape[1],0:2] = np.transpose(indices)
    ...
    for i_freq in numba.prange(energy_0.shape[-1]):
        queue[0:energy_0.shape[-1],2] = energy_0[0:energy_0.shape[-1],i_freq]
...
def _energy_exchange(
    ...
    while queue[i,-1] > 0:
        ...
        if appendix.shape[0] != 0:
            queue[last:last+appendix.shape[0],:] = appendix
            last += appendix.shape[0]
