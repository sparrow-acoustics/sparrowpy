...
def _calculate_energy_exchange_queue(
        queue, form_factors_tilde, patch_distances, patch_receiver_energy,
          patch_receiver_distance, ir, i_freq):
    ...
    while queue.shape[0] > 0:
        ...
        appendix, ir = _shoot(row, form_factors_tilde, patch_distances,
                             patch_receiver_energy, patch_receiver_distance, ir, i_freq)
...
def _shoot(row_in, form_factors_tilde, patch_distances, patch_receiver_energy,
    ...
    ej = e * form_factors_tilde[h,i,:] # FOR THIS TO WORK
                                       # visibility must be encoded in ff tilde -- João

    eR = ej * patch_receiver_energy

    j = np.nonzero(eR > thres)

    dR = d+patch_receiver_distance[j]

    ir = _collect_receiver_from_queue(ir, i_freq, energy=eR[j], distance=dR)

    queue_out = np.empty((j.shape[0], 4))

    queue_out[:,0] = i*np.ones_like(j)
    queue_out[:,1] = j
    queue_out[:,2] = ej[j]
    queue_out[:,3] = d+patch_distances[i,j]
