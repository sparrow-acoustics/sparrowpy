...
import numba
import numpy as np
from .universal_ff.univ_form_factor import pt_solution_receiver as patch_receiver_energies
...
def _universal(receiver_pos: np.ndarray, patch_points: np.ndarray,
               patch_areas: np.ndarray, patch_receiver_distance: np.ndarray,
    ...
    for i in range(patch_receiver_distance.shape[0]):
        ...
        receiver_factor[i,:] = patch_receiver_energies(receiver_pos, patch_points[i], patch_areas[i]) * np.exp(-air_attenuation * patch_receiver_distance)
