...
class Polygon:
    ...
    def __init__(
        self, points: np.ndarray, up_vector: np.ndarray, normal: np.ndarray
    ) -> None:
    ...
    def intersection(
        ...
        if _cmp_floats(np.dot(direction, n), 0.0):
            return None

        t = (
            1
            / (np.dot(direction, n))
            * (np.dot(n, self.pts[0]) - np.dot(n, origin))
        )
