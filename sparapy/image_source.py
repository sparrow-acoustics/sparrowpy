...
class ImageSource():
    ...
    def calculate_image_sources(WallsHesse, SourcePos, MaxOrder):
       ...
       n_IS = 1 + n_walls / (n_walls - 2) * ((n_walls - 1) ** MaxOrder - 1)

       ISList = np.zeros(n_IS, dtype=object)

       ISList = [{
            'Position': SourcePos,  # Set the Position to the value of SourcePos
            'Walls': [],            # Initialize Walls as an empty list
            'Order': 0              # Set Order to 0
        }]
       
       list_end = 1

       for order in range(1, MaxOrder + 1):
            for iISID in range(list_end):
                if ISList[iISID]['Order'] == order - 1:
                    for iWalls in range(n_walls):
                        n0 = WallsHesse[iWalls, 0:3]
                        p = ISList[iISID]['Position']
                        d = WallsHesse[iWalls, 3]
                        t0 = d - np.dot(n0, p)
                        ...
                        if wallCriterium and (t0 > 0):
                            new_position = p + 2 * t0 * n0
                            new_walls = ISList[iISID]['Walls'] + [iWalls]
                            new_order = order

                            ISList.append({'Position': new_position, 'Walls': new_walls, 'Order': new_order})
                            list_end += 1
